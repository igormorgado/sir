...
def timeit_plot2D(data, axes=None, xlabel='xlabel', title='title', **kwargs):
    ...
    ylabel = kwargs.get('ylabel', 'Tempo (s)')
...
def timeit_heatmap(data, xlabel='xlabel', ylabel='ylabel', **kwargs):
    ...
    for k, v in data.items():
        ...
        fig.colorbar(hmap).set_label("Tempo(s)")
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
